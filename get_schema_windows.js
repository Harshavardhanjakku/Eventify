// Windows-compatible JavaScript file to get all tables with their columns and constraints
// Run this in Node.js with pg (PostgreSQL client)

const { Pool } = require('pg');

// Database connection - update these values for your setup
const pool = new Pool({
  user: 'postgres',
  host: 'localhost',
  database: 'eventifydb',
...
async function getAllTablesSchema() {
  ...
  try {
    ...
    console.log('\nMake sure to update the database connection details in this file:');
    ...
    console.log('- host: your PostgreSQL host (usually localhost)');
    console.log('- port: your PostgreSQL port (usually 5432)');
  } finally {
    client.release();
    await pool.end();
  }
